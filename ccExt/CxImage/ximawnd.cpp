 ...
 * \return true if everything is ok
 */
bool CxImage::CreateFromHBITMAP(HBITMAP hbmp, HPALETTE hpal, int bmBitsPixel)
{
	if (!Destroy())
		return false;
	...
	if (hbmp) { 
		...
		if		(bmBitsPixel < 1)	bmBitsPixel = bm.bmBitsPixel;
		else if (bmBitsPixel <= 1)	bmBitsPixel = 1;
		else if (bmBitsPixel <= 4)	bmBitsPixel = 4;
		else if (bmBitsPixel <= 8)	bmBitsPixel = 8;
		else if (bmBitsPixel <= 16)	bmBitsPixel = 16;
		else if (bmBitsPixel <= 24)	bmBitsPixel = 24;
		else bmBitsPixel = 32;
        if (!Create(bm.bmWidth, bm.bmHeight, bmBitsPixel, 0))
			return false;
...
void CxImage::InitTextInfo( CXTEXTINFO *txt )
{
    ...
    _tcscpy( txt->text, _T("Sample Text 01234")); // text use TCHAR mappings <Cesar M>
    txt->align = DT_CENTER;
    return;
}
